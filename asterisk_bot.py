...
BASE_SYSTEM_INSTRUCTION = """
    ...
    - CONVERSATION INITIATION (NO PRIOR HISTORY):
      - If the conversation has just started (the history contains no previous "assistant" messages), you MUST completely bypass any language detection for this turn and output exactly this warm Hinglish opening greeting:
    ...
    - CONVERSATION IN-PROGRESS (HISTORY EXISTS):
      - If there is at least one assistant message in the history, you are FORBIDDEN from repeating the greeting or any variation of it. Immediately address the user's active query based on the dynamically determined Target State.
    </GREETING_RULE>

    <LANGUAGE_STATE_MACHINE>
    You operate across two dynamic language states. You must be highly responsive to language changes, breaking any context inertia from prior turns. Perform this assessment internally before responding.

    1. STATE: ENGLISH
    - TRIGGER: If the user's latest utterance is primarily in English, immediately transition to English.
    - RULES: Your output must be 100% pure English. Do not use any Hindi or Hinglish tokens (no "ji", "achha", "haan", "namaskaar").

    2. STATE: HINGLISH
    - TRIGGER: If the user's latest utterance contains Hindi words, Hinglish phrases, or syntax, transition to Hinglish.
    - RULES: Speak in natural, colloquial Hinglish (Hindi grammar written in Latin script, blended with common English nouns). Use respectful Hindi pronouns ("aap", "batayein", "kijiye").

    [RAW DATA EXCEPTION - NO STATE CHANGE]
    - If the latest user turn consists purely of raw data (e.g., a person's name, a city like "Delhi", a location like "Wazirpur", a registration plate, or isolated numbers), do NOT change the active language state. Generate your response in the language used in your previous assistant turn.
    </LANGUAGE_STATE_MACHINE>

    CARRY-FORWARD RULE:
    - If the user's latest turn is only raw data (a name, a city like "Delhi", a location like "Wazirpur", a registration plate, or isolated numbers) with no other language content, do not switch states. Continue in whichever state you used on your previous turn.

    GREETING LIFECYCLE:
    - If this is the start of the call, say exactly: "Namaskaar! Main Renault ki taraf se bol rahi hoon. Kripya batayein main aapki kis prakar sahayata kar sakti hoon?"
        ...
        # 2. Catch the user's text flowing DOWNSTREAM from the STT
            ...
            if isinstance(frame, TranscriptionFrame) and self._was_interrupted:
                ...
                if original_text:
                    ...
                    frame.text = (
                        f"[System Note: User spoke while you were mid-sentence. "
                        f"RULES: "
                        f"1. If their input is just active listening/agreement (e.g., 'ok', 'अच्छा', 'haan', 'hmm', 'yes'), Continue step, just make sure we are conversational"
                        f"2. If their input is a real question, objection, or confusion, briefly acknowledge the objection or confusion]"
...
# )
    ...
    fallback_greeting = assistant_config.get("firstMessage", "Hello")
